...
* `charset`
  ...
  这个属性是让浏览器根据编码去解码对应的 html 内容，文件不一定是这个编码。统一使用`UTF-8`
...
* **行内元素**（内联元素），常见的有：a、strong、b、em、i、del、s、ins、u、span等
...
* **行内块元素**，**img、input、td** 同时具有块级元素和行内元素的特点，资料中并没有提及
...
`<u>`<u>下划线</u>`</u>` ；`<ins>`<u>下划线</u>`</ins>`，推荐使用

`<s>`~~删除线~~`</s>`；`<del>`~~删除线~~`</del>` ，推荐使用
...
表格的现在还是较为常用的一种标签，但不是用来布局，常用**显示、展示表格式数据。**
...
* 之前使用的`cellspacing`（单元格之间距离）、`cellpadding`（内容与单元格边框距离）应该使用CSS实现
...
`<dl>` `<dt>` `<dd>`   **列表**带有**项目**和**描述**。常见于页面下面帮助部分。
...
- **text**：文本，使用 action 提交后台需指定 **name** 属性
- **password**：密码，使用 action 提交后台需指定 **name** 属性
...
> label标签主要目的是为了提高用户体验，为 input 元素定义标注（标签）。用于绑定一个表单元素，当点击label标签的时候, 被绑定的表单元素就会获得输入**焦点**
